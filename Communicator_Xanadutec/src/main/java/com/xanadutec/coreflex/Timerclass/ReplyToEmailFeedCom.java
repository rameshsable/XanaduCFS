...
public class ReplyToEmailFeedCom {

	public void sendEmailReply(String actualSubject, List<String> emailTo, List<String> emailCC, String attachment,
			String ContentsOfMessage, HttpServletRequest request, FileUploadService fileUploadService, EmailSendAndReceivePermissionService emailSendAndReceivePermissionService) {
					...
					try {
						...
						if (subject.equalsIgnoreCase(actualSubject)) {
							...
							replyMessage.setRecipients(Message.RecipientType.TO,
									InternetAddress.parse(bufTo.toString(), false));
							replyMessage.setRecipients(Message.RecipientType.CC,
									InternetAddress.parse(bufCC.toString(), false));
							try {
							...
							// Send the message by authenticating the SMTP
							// server
							// Create a Transport instance and call the
							// sendMessage
